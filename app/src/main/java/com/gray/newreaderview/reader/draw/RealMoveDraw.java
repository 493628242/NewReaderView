...
import com.gray.newreaderview.R;
...
/**
 * @author wjy on 2018/6/13.
 */
public class RealMoveDraw extends Draw {
...
//    private Bitmap pathCContentBitmap;//C区域内容Bitmap
    ...
    public RealMoveDraw(PageProperty pageProperty, ReaderView view) {
    ...
    private void init(Context context) {
        viewHeight = UIUtils.getDisplayHeight(context);
        viewWidth = UIUtils.getDisplayWidth(context);
        a = new MyPoint();
        f = new MyPoint();
        g = new MyPoint();
        e = new MyPoint();
...
//        pathBPaint.setColor(mReaderView.getContext().getResources().getColor(R.color.blue_light));
...
//    }
    ...
    private void draw(Canvas canvas, Bitmap mPreviousBM, Bitmap mCurrentBM, Bitmap mNextBM) {
        if (mPreviousBM == null && mNextBM == null) {
            return;
        }
        if (a.x == -1 && a.y == -1) {
            drawPathAContent(canvas, getPathDefault(),
                    mPreviousBM == null ? mCurrentBM : mPreviousBM);
        } else {
            if (f.x == viewWidth && f.y == 0) {
                ...
                drawPathBContent(canvas, getPathAFromTopRight(),
                        mNextBM == null ? mCurrentBM : mNextBM);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mDownY = event.getY();
                style = STYLE_RIGHT;
//                if (mDownX <= viewWidth / 3) {//左
//                    style = STYLE_LEFT;
//
//                } else if (mDownX > viewWidth / 3 && mDownY <= viewHeight / 3) {//上
//                    style = STYLE_TOP_RIGHT;
//
//                } else if (mDownX > viewWidth * 2 / 3 && mDownY > viewHeight / 3 && mDownY <= viewHeight * 2 / 3) {//右
//                    style = STYLE_RIGHT;
//
//                } else if (mDownX > viewWidth / 3 && mDownY > viewHeight * 2 / 3) {//下
//                    style = STYLE_LOWER_RIGHT;
//
//                } else if (mDownX > viewWidth / 3 && mDownX < viewWidth * 2 / 3 && mDownY > viewHeight / 3 && mDownY < viewHeight * 2 / 3) {//中
//                    style = STYLE_MIDDLE;
//                }
                break;
            case MotionEvent.ACTION_MOVE:
                ...
                if (mOffsetX > 0) {
                    if (mAdapter.canMoveToPrevious()) {
                        ...
                        Toast.makeText(mReaderView.getContext(),
                                "已到第一页", Toast.LENGTH_SHORT).show();
                    }
                } else if (mOffsetX < 0) {
                    if (mAdapter.canMoveToNext()) {
                        ...
                        Toast.makeText(mReaderView.getContext(),
                                "已到最后一页", Toast.LENGTH_SHORT).show();
                    }
                }
                break;
    ...
    public void onDraw(Canvas canvas, Bitmap mPreviousBM, Bitmap mCurrentBM, Bitmap mNextBM) {
        if (mOffsetX > 0) {
            draw(canvas, mPreviousBM, mCurrentBM, null);
        } else if (mOffsetX < 0) {
            draw(canvas, null, mCurrentBM, mNextBM);
        } else if (mOffsetX == 0) {
            canvas.drawBitmap(mCurrentBM, 0, 0, null);
        }
    }
    ...
    public void moveToNext(Canvas canvas, Bitmap mCurrentBM, Bitmap mNextBM, int currX, int currY) {
        draw(canvas, null, mCurrentBM, mNextBM);
        setTouchPoint(currX, currY, style);
    }
...
//                mReaderView.postInvalidate();
     ...
     * @param pathA
     */
    private void drawPathAContent(Canvas canvas, Path pathA, Bitmap bitmap) {
        canvas.save();
...
//        mPath.lineTo(i.x,i.y);
     ...
     * @param pathA
     */
    private void drawPathBContent(Canvas canvas, Path pathA, Bitmap bitmap) {
        canvas.save();
     ...
     * @param pathA
     */
    private void drawPathCContent(Canvas canvas, Path pathA, Bitmap bitmap) {
        canvas.save();
        canvas.clipPath(pathA);
